...
`.reviews/` holds per-PR reviews, `.checks/` holds whole-repo hygiene scans
(dead-code, dependency drift, …) run weekly/monthly or on demand — **not**
per-commit gates.

```
.checks/<kind>/<short_sha>.md     # one run per main-HEAD commit; newest-first by `generated`
```
