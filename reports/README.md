...
add `reports/` to `.gitignore` — the Docs tab still renders them.
...
- `reports/auto-docs/`   → `auto-docs`
- `reports/drift/`       → `drift-auditor`
- `reports/coverage/`    → `coverage`
- `reports/deps-quality/` → `deps-quality`
- `reports/perf/`        → `perf`
- `reports/health/`      → `health`
- `reports/dead-code/`   → `dead-code` (legacy `/check` kind)
