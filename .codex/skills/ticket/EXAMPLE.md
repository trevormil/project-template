...
- `acceptance` — optional **block list** of strict, checkable criteria defining
  a correct/best implementation (one `- "…"` per line; criteria may contain
  commas). **Required before running more than one implementation lane**:
  with N>1 lanes (parallel variant attempts), each lane is gated on tests
  passing AND every criterion met (judged per-criterion with evidence), then
  eligible lanes are ranked by `/code-review` overall score (lower `risk_score`
  tie-breaks); the winner's MR is surfaced for human merge. Lane count is chosen
  by the implementer at launch, not stored on the ticket. Default 1.
